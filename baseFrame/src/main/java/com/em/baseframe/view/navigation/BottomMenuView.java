...
public class BottomMenuView extends LinearLayout {
    ...
    private void initData() {
        ...
        mGroup.setBackgroundColor(mContext.getResources().getColor(R.color.app_bot_bg));
    ...
    public void init(List<Drawable> drawables, int color, List<BaseLazyFragment> fgts, List<String> menus, List<Drawable> drawableBgs,  FragmentManager fm) {

        for (int i = 0; i < drawables.size(); i++) {
            RadioButton button = getRadioButton(drawables, color, menus, drawableBgs, i);
    ...
    @NonNull
    private RadioButton getRadioButton(List<Drawable> drawables, int color, List<String> menus, List<Drawable> drawableBgs, int i) {
        RadioButton button = new RadioButton(mContext);
        button.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
        button.setTextColor(mContext.getResources().getColorStateList(color));
        button.setText(menus.get(i));
        button.setBackgroundColor(mContext.getResources().getColor(R.color.app_bot_bg));
        Bitmap a = null;
        button.setButtonDrawable(new BitmapDrawable(a));
        button.setBackground(drawableBgs.get(i));
