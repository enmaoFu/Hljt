...
import com.hljt.app.adapter.ChatGroupAdapter;
...
/**
 * @title  应用辅助软件-》专家在线-》聊天页面 群聊 activity
 * @date   2017/09/27
 * @author enmaoFu
 */
public class ChatGroupActivity extends BaseAty implements TextWatcher {
    ...
    //数据源
    private List<ChatGroupPojo> chatGroupPojos;
    //是否已经弹出更多
    ...
    //消息实体类
    private ChatGroupPojo chatGroupPojo;
    //数据库
    ...
    private Handler mHandler = new Handler(){
        public void handleMessage(Message msg){
            ...
            switch (msg.what){
                case 1:
                    //发送消息成功后，将发送消息加入列表
                    chatGroupPojo = new ChatGroupPojo("https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1507083360&di=681541dec85bd70ec122584074d6eb92&imgtype=jpg&er=1&src=http%3A%2F%2Fwww.dxs518.cn%2Fimagesfiles%2Farticle%2F615%2F201628%2F12_xfny0___jpg.gif",
                            getInputMessage,TYPE_SEND,YES_RECORD);
                    chatGroupAdapter.addData(chatGroupPojo);
                    //发送消息成功后清理输入框
                    inputMessage.setText("");
                    //将recyclerview滚动到最底部
                    rv_data.scrollToPosition(chatGroupAdapter.getItemCount() - 1);
                    ...
                    if(conversationPojos.size() == 0){
                        ...
                        conversationPojo.setConversationId(id);
    ...
    protected void initData() {
        ...
        //实例化适配器
        chatGroupAdapter = new ChatGroupAdapter(R.layout.item_chat_group, new ArrayList<ChatGroupPojo>());
        //设置布局管理器
        rv_data.setLayoutManager(mLayoutManager);
        //大小不受适配器影响
        rv_data.setHasFixedSize(true);
        //设置加载动画类型
        chatGroupAdapter.openLoadAnimation(BaseQuickAdapter.ALPHAIN);
        //设置删除动画类型
        rv_data.setItemAnimator(new DefaultItemAnimator());
        //设置adapter
        rv_data.setAdapter(chatGroupAdapter);
        //将recyclerview滚动到最底部
        rv_data.scrollToPosition(chatGroupAdapter.getItemCount() - 1);
        //监听输入框变化，切换右边按钮的切换
            ...
            case R.id.add:
                ...
                if(getAddText.length() == 0){
                    ...
                    if(getInputMessage.length() == 0){
                        ...
                        EMMessage message = EMMessage.createTxtSendMessage(getInputMessage, id);
                        //设置为群聊
    ...
    public void getMessageNumber(){
        //获取消息总数
        EMConversation messageNumber = EMClient.getInstance().chatManager().getConversation(id);
        /**
        ...
        try{
            ...
            if(number != 0){
                ...
                for(int i = 0; i < messageList.size(); i++){
                    ...
                    chatGroupPojos.add(chatGroupPojo);
                }
                chatGroupAdapter.setNewData(chatGroupPojos);
            }
        }catch (Exception e){
            chatGroupPojos = new ArrayList<>();
            chatGroupPojo = new ChatGroupPojo("https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1507083360&di=681541dec85bd70ec122584074d6eb92&imgtype=jpg&er=1&src=http%3A%2F%2Fwww.dxs518.cn%2Fimagesfiles%2Farticle%2F615%2F201628%2F12_xfny0___jpg.gif",
